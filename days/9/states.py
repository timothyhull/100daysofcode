...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    abbreviation = us_state_abbrev.get(state_name.title(), NOT_FOUND)
...
def get_longest_state(data):
    ...
    for state in list(data):
        state_len = len(state)
        if state_len > state_name_length:
            state_name_length = state_len
            state_name = state
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    states.sort()
    first_ten_states = sorted(list(us_state_abbrev.values())[:10])
    last_ten_states = states[-10:]
    states = first_ten_states + last_ten_states
